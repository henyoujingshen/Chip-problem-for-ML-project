...
parameters = {'seed': 100, 'nthread': 5, 'gamma': 0, 'lambda': 0.1,
...
def custom_function(iter, maxiter):
    rang = [0.01, 0.99]
    x = (rang[1] * iter + rang[0] * (maxiter - iter) - (rang[0] + rang[1]) * maxiter / 2) / maxiter * 10 / (rang[1] - rang[0])
    fx = 1.0 / (1 + np.exp(-x))
    return fx
