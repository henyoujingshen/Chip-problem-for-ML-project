...
computations = 100
sample_init = int(problem_param['dimension']) * 15
...
def testFunc(sample_array):
    ...
    if problem_param['name'] == 'rosenbrock':
        ...
        x1, x2 = X[:, 0], X[:, 1]
        a = x1 * x2
        result = (4 - 2.1 * np.square(x1) + np.power(x1, 4)/3) * np.square(x1) + x1 * x2 + (-4 + 4 * np.square(x2)) * np.square(x2)
        problem_param['column_name'] = ['x1', 'x2', 'y']
...
def latin_hypercube_sampling(num_samples):
    if type(problem_param['range'][0]) != int and type(problem_param['range'][0]) != float:
