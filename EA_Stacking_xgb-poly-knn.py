...
import stacking_3model_spearman as mt
...
def mutation(individual, pb):
    ...
    switch = np.random.uniform(size=(1, 5))
    switch = np.rint(2 * pb * switch).ravel()
    step = np.random.uniform(size=(1, 5))
    step = np.abs(np.rint(5 * step - 0.5)).ravel()
    full_mut = np.multiply(scale, step) + offset

    comp = full_mut - individual[0] + 1 - switch
    while ~comp.all():
        nonzero_index = np.array(np.where(comp == 0))
        resample_mut = np.random.uniform(size=(1, nonzero_index.size))
        resample_mut = np.abs(np.rint(5 * resample_mut - 0.5))
        full_mut[nonzero_index] = np.multiply(scale[nonzero_index], resample_mut) + offset[nonzero_index]
        comp = full_mut - individual[0] + 1 - switch
        # print("invalid mutation detected")

    result = np.multiply(switch, full_mut) + np.multiply(1 - switch, np.array(individual[0]))
    individual[0] = result
    return individual
...
def monte_carlo_sampling(num_samples):
    global seed
    num_dimensions = 5
    k = np.array(lhsmdu.createRandomStandardUniformMatrix(numDimensions=num_dimensions,
...
def popEvaluate(base_model, meta_model, model_num, weight, base_model_weight, population):
    ...
    if model_num == 4:
        ...
        with open('poly' + str(model_num) + '.model', 'rb') as file:
            model_poly = pickle.load(file)
        pop_pred_poly = np.abs(model_poly.predict(pop_array))
        # knn base model
        with open('knn' + str(model_num) + '.model', 'rb') as file:
            model_knn = pickle.load(file)
        pop_pred_knn = np.abs(model_knn.predict(pop_array))
...
def iterate(Sample_Train, Sample_Points, Data, num_boost_round):
    ...
    for generation in range(0, generations):
        ...
        # 采样点训练XGB代理模型
        # print(generation)
        base_model, meta_model, model_num, weight, base_model_weight = mt.modelTrain(Sample_Train, Data, parameters, num_boost_round, generation)
        # 评估
        # 种群个体fitness评估
        popEvaluate(base_model=base_model, meta_model=meta_model, model_num=model_num, weight=weight, base_model_weight=base_model_weight, population=pop)
        ...
        popEvaluate(base_model=base_model, meta_model=meta_model, model_num=model_num, weight=weight, base_model_weight=base_model_weight, population=pop_children)
        ...
        pop_array, pop_pred = popEvaluate(base_model=base_model, meta_model=meta_model, model_num=model_num, weight=weight, base_model_weight=base_model_weight, population=pop)
