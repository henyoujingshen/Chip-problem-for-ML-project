...
import stacking_model_v6 as mt

# 超参数定义
from sklearn.cluster import KMeans


pop_init = 3000  # 初始种群大小
num_cluster = [1, 1]  # 种群聚类个数
threshold = [60, 100]

cross_pb = 0.6  # 交叉概率
mutation_pb = 0.7
select_num = 1500  # 从父代中选择出的育种个体数量
...
def iterate(Sample_Train, Sample_Points, num_boost_round):
    ...
    for generation in range(0, generations):
        ...
        for i in range(0, len(pop_cross) - len(pop_cross) % 2, 2):
            toolbox.Crossover(ind1=pop_cross[i], ind2=pop_cross[i + 1], alpha=cross_pb, xu=-5, xl=5)
        # 变异
        # pop_mutation = [toolbox.clone(individual) for individual in pop_cross]
        pop_mutation = pop_cross
        for ind in pop_mutation:
            toolbox.Mutation(individual=ind, mu=0, sigma=1, indpb=mutation_pb, xu=-5, xl=5)
        # 评估新产生的个体
...
if __name__ == '__main__':
    ...
    # 定义突变函数
    toolbox.register("Mutation", tools.mutGaussian)
    # 定义交叉函数
    toolbox.register("Crossover", deap.tools.cxBlend)
