...
problem_param = {
    # 'name': 'rosenbrock', 'rastrigin', 'griewank'
    'name': 'rosenbrock',
    # 'dimension': 6, 8, 10
    'dimension': 6,
    # 'range': [-5, 10], [-5, -5], [-600, 600]
    'range': [-5, 10],
    # 'range': [0, 1],
}
...
def mutation(individual):
    ...
    if prob > mutation_param['mutation_prob']:
        ...
        # Sigma = Sigma - 0.5
        xo = xp + Sigma * (xu - xl)
        # if xo.any() <= xl or xo.any() >= xu:  # x offspring
            # print("Bound detected")
...
def testFunc(sample_array):
    ...
    if problem_param['name'] == 'rosenbrock':
        ...
        result = 10 * problem_param['dimension'] + np.sum(np.square(X) - 10 * np.cos(2 * np.pi * X))
        ...
        result = np.sum(np.square(X)) / 4e3 - np.prod(np.cos(np.multiply(X, den))) + 1
...
if __name__ == '__main__':
    ...
    print("Mean value:", np.mean(Optimum))
